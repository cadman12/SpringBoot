Spring Boot 서버에서 자체 서명된 SSL 인증서를 사용하여 테스트를 실행하려면 다음 단계를 따르세요.

1. 자체 서명된 인증서 생성
아직 생성하지 않은 경우 자체 서명된 인증서를 생성해야 합니다. OpenSSL을 사용하여 인증서를 생성하고 이를 Spring Boot에서 사용할 수 있는 JKS(Java Keystore) 형식으로 변환할 수 있습니다.
...
요약
다음 단계에 따라 HTTPS 연결 테스트를 위해 자체 서명된 SSL 인증서를 사용하도록 Spring Boot 애플리케이션을 구성할 수 있습니다. 이 설정은 Spring Boot 애플리케이션이 개발 또는 테스트 환경에서 SSL/TLS 연결을 적절하게 처리할 수 있도록 보장합니다.