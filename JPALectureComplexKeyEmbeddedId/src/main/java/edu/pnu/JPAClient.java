...
public class JPAClient {
	...
	public void insertData(EntityManager em) {
		...
		Member m1 = Member.builder().memberID(MemberID.builder().id(1L).username("홍길동").build())
				.password("1234").email("hong@kildong").role("ROLE_USER").build();
		em.persist(m1);
		Member m2 = Member.builder().memberID(MemberID.builder().id(1L).username("이순신").build())
				.password("1234").email("lee@sunsin").role("ROLE_MANAGER").build();
		em.persist(m2);
